...
class MainFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        busStopAdapter = BusStopAdapter(emptyList(), object : BusStopAdapter.OnItemClickListener {
            override fun onItemClick(schedule: Schedule) {
                findNavController().navigate(MainFragmentDirections.actionMainFragmentToDetailsFragment(schedule.stopName))
            }
        })
        recyclerView.adapter = busStopAdapter

        viewModel.fullSchedule().observe(viewLifecycleOwner) { busStopAdapter.updateData(it) }
